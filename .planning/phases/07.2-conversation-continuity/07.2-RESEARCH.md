...
const args = [
  ...
  '--append-system-prompt', systemPrompt,
  '--mcp-config', mcpConfigPath,
  '--settings', settingsJson,  // NEW: inline hooks config
];
```
...
server.tool(
  ...
  async ({ query, limit, days_back, include_conversations }) => {
    const embeddingResults = await semanticSearch(query, { topK: limit, daysBack: days_back });

    if (include_conversations) {
      const conversationResults = await searchConversations(query, { limit, daysBack: days_back });
