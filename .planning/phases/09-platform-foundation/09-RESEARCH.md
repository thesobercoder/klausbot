...
function displayStartupChecklist(results: CheckResult[]): void {
  ...
  for (const { capability, status } of results) {
    const icon = status === 'ok' ? theme.colors.green('\u2713')
               : status === 'degraded' ? theme.colors.yellow('\u26A0')
               : theme.colors.red('\u2717');
    const name = capability.name;
    const statusText = status === 'ok' ? 'enabled'
...
import { capabilities, checkAllCapabilities, Status } from '../platform/capabilities.js';
