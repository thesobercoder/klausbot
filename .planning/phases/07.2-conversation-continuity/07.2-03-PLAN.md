...
<task type="auto">
  ...
  <action>
Add getHooksConfig() function after getMcpConfig():
...
<task type="auto">
  ...
  <files>src/daemon/spawner.ts</files>
  <action>
In queryClaudeCode(), update the args array construction:

1. After `const mcpConfigPath = writeMcpConfigFile();` add:
```typescript
...
2. Update args array to include --settings:
```typescript
